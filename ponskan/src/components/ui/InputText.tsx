...
export const InputText = forwardRef<HTMLInputElement, Props>(({ textLabel, error, inputId, visible, ...props }, ref) => {
    return (
        <div className={`flex flex-col h-14 w-full gap-2 mb-2 ${visible ? "visible" : "hidden"}`}>
            <div className=" relative h-fit w-full ">
                <input id={inputId} ref={ref} {...props} placeholder=" " className={` peer px-3 w-full h-12 text-slate-600 rounded-xl border focus:border-amber-400 transition-colors outline-none ${!error ? "border-slate-300" : "border-red-500"}`} /> 
